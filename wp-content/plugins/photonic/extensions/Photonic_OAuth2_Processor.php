...
abstract class Photonic_OAuth2_Processor extends Photonic_Processor {
	...
	function refresh_token($provider, $refresh_token) {
		$token = $this->get_access_token_from_refresh($provider, $refresh_token, false);
		if (!empty($token)) {
			$secret = md5($this->client_secret, false);
			if (isset($_COOKIE['photonic-' . $secret . '-oauth-token'])) {
				unset($_COOKIE['photonic-' . $secret . '-oauth-token']);
	...
	function get_access_token_from_refresh($provider, $refresh_token, $save) {
		$token = array();
		$response = Photonic::http($this->access_token_url(), 'POST', array(
			'client_id' => $this->client_id,
			'client_secret' => $this->client_secret,
		...
		if (!is_wp_error($response)) {
			...
			$photonic_authentication[$provider] = $token;
			if ($save) {
